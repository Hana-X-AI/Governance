...
### Requirement #1: Server Resources

**Cross-References to Phase 3.2 Tasks**:
- **T-022** (line 530): "Ensure at least 20GB free before build"
- **T-024**: Requires `systemd-run --property=MemoryMax=4G` for build
- **T-023**: pnpm install creates ~2.5GB node_modules directory
...
**Classification**: **BLOCKING for Phase 3.2** ✅
...
- **FR-008**: Reverse Proxy Configuration (Nginx)
- **File Path**: `/etc/nginx/sites-available/n8n.conf`

**Failure Mode**:
```bash
# If Nginx not installed:
sudo nano /etc/nginx/sites-available/n8n.conf
# Output: /etc/nginx: No such file or directory

sudo systemctl reload nginx
# Output: Unit nginx.service could not be found
# Exit code: 5 (NOT_FOUND)
```

**Classification**: **BLOCKING for Phase 4** ✅

---

### Requirement #3: Source Code Transfer

**Cross-References to Phase 3.2 Tasks**:
- **T-021**: Clone n8n Repository (expects source at `/opt/n8n/build/`)
- **T-023**: pnpm install (requires `package.json`)
- **T-024**: pnpm build:deploy (requires `pnpm-workspace.yaml`)

**Failure Mode**:
```bash
# If source not transferred:
cd /opt/n8n/build
pnpm install
# Output: No package.json found in /opt/n8n/build
# Exit code: 1 (FAILURE)
```
...
**Cross-References to Specification**:
- **FR-004**: Systemd service requires `EnvironmentFile=/opt/n8n/.env`
- **FR-010**: Lists DB_POSTGRESDB_* variables as required

**Failure Mode**:
```bash
# If .env missing:
sudo systemctl start n8n.service
# journalctl -u n8n.service:
# Output: Error: Missing required environment variable: DB_POSTGRESDB_HOST
# Output: Error: Cannot connect to database
# Service status: failed (exit code 1)
```

**n8n Source Code Verification**:
```typescript
// From n8n source: packages/cli/src/databases/config.ts
if (!process.env.DB_POSTGRESDB_HOST) {
  throw new Error('DB_POSTGRESDB_HOST environment variable is required');
}
```
