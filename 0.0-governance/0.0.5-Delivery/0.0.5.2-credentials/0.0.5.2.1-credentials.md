**Document Type**: Delivery - Credentials & Secrets Inventory  
**Created**: 2025-10-27  
**Topic**: HX.DEV.LOCAL Authentication Credentials  
**Purpose**: Centralized storage of all authentication credentials and secrets for infrastructure  
...
- **Service:** CodeRabbit AI Code Review Platform
- **Purpose:** AI-assisted code review and analysis
- **CLI Version:** 0.3.4
- **Installed On:** hx-cc-server.hx.dev.local (192.168.10.224)
- **Installation Path:** `/home/agent0/.local/bin/coderabbit`
- **Environment Variable:** `CODERABBIT_API_KEY`
- **Authentication Method:** OAuth (browser-based login required)
- **Deployed:** November 10, 2025
- **CLI Documentation:** https://cli.coderabbit.ai
...
coderabbit review --plain --config coderabbit.yaml
...
- CLI version 0.3.4 installed on November 10, 2025
- CLI requires OAuth login via browser: `coderabbit auth login`
- **IMPORTANT**: CLI uses OAuth authentication (browser-based), NOT API key directly
- API key stored for reference only (CLI uses OAuth tokens stored in config)
- Installed on hx-cc-server (192.168.10.224) for Layer 3 integration
- **Manual Step Required**: User must run `coderabbit auth login` in interactive session to authenticate

**Integration Points:**
- Layer 3: Code review automation for Governance repository
- Server: hx-cc-server.hx.dev.local (192.168.10.224)
- MCP Server: hx-coderabbit-server.hx.dev.local (192.168.10.228) - separate service
...
### 11. svc-n8n PostgreSQL Service Account (Database-Only - NOT Domain Account)

**Username**: `svc-n8n`
**Password**: `Major8859` (NO special characters - N8N URL compatibility)
**Type**: PostgreSQL database role ONLY (NOT OS/domain account)
**Created**: November 8, 2025
**Database**: `n8n_poc3` on hx-postgres-server (192.168.10.209)

**Database Role Password**: `Major8859` (NO special characters)

**✅ PASSWORD VERIFIED**: Database role password confirmed on 2025-11-08
- Tested from hx-n8n-server (192.168.10.215) → hx-postgres-server (192.168.10.209)
- Connection successful to n8n_poc3 database
- Authentication working correctly
- NO special characters to avoid N8N connection string URL encoding issues

**⚠️ DEDICATED FOR N8N APPLICATION**:
- **CREATED FOR**: N8N workflow automation database connections only
- **PURPOSE**: Avoid special character issues in N8N database connection URLs
- **PASSWORD**: Major8859 (no exclamation mark or other special characters)
- Created: November 8, 2025 (POC3 deployment)
- Reason: N8N TypeORM cannot handle special characters in DATABASE_URL properly

**⚠️ IMPORTANT**: PostgreSQL database role ONLY, NOT a domain/OS account

**Usage**:
- Dedicated service account for N8N application database connections
- PostgreSQL database access with full privileges on n8n_poc3 database
- Used by N8N systemd service on hx-n8n-server (192.168.10.215)

**Database Role Creation**:
```bash
# Created on hx-postgres-server via psql:
CREATE ROLE "svc-n8n" WITH LOGIN PASSWORD 'Major8859';
GRANT CONNECT ON DATABASE n8n_poc3 TO "svc-n8n";
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO "svc-n8n";
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO "svc-n8n";
GRANT CREATE ON SCHEMA public TO "svc-n8n";
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO "svc-n8n";
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO "svc-n8n";
```

**Connection Strings (RECOMMENDED FOR N8N)**:
```bash
# Separate environment variables (RECOMMENDED - no URL encoding issues)
DB_TYPE=postgresdb
...
# Alternative: Connection URL format (also works with URL-safe password)
postgresql://svc-n8n:Major8859@hx-postgres-server.hx.dev.local:5432/n8n_poc3
...
**Database Permissions**:
- ✅ CONNECT on n8n_poc3 database
- ✅ ALL PRIVILEGES on all tables in schema public (50 tables)
- ✅ ALL PRIVILEGES on all sequences in schema public
- ✅ CREATE on schema public (for migrations)
- ✅ Default privileges for future tables and sequences

**Best Practice**:
- ✅ N8N application → Use `svc-n8n` (URL-safe password)
- ✅ LiteLLM application → Use `svc-litellm` (URL-safe password)
- ✅ Other applications → Use `svc-postgres@hx.dev.local` (standard password with special chars)
- ✅ Administration → Use `postgres@hx.dev.local`

**Security Notes**:
- ✅ Dedicated account for single application (isolation)
- ✅ Password has NO special characters (N8N TypeORM compatibility)
- ⚠️ Password is simpler (no special chars) - acceptable for dev environment
- ⚠️ Database role only - no OS/domain account needed for N8N
- ✅ Follows same pattern as svc-litellm (lines 610-699)

**Deployment Reference**:
- POC3 n8n Deployment - Phase 3 completion (2025-11-08)
- Documentation: `/srv/cc/Governance/x-poc3-n8n-deployment/p3-execution/PHASE3-COMPLETION-SUMMARY.md`

---

### 12. crawl4ai Service Account (Domain-Integrated - Samba LDAP/DC)
...
### 14. n8n Service Account (Domain-Integrated - Samba LDAP/DC)

**Username**: `n8n@hx.dev.local`
**Password**: `Major8859!`
**Type**: Domain service account (Samba LDAP/DC - NOT local user)
**UID**: TBD (Samba DC auto-assigned)
**GID**: `1114200513` (Domain Users)
**Home**: `/home/n8n@hx.dev.local`
...
- Domain-integrated authentication via Kerberos + SSSD
- Used for service operations on hx-n8n-server (192.168.10.215)
- Account available on ALL domain-joined servers
- Service deployment: POC3 (In Progress)
...
- **Port**: 5678 (internal), 443 (Nginx reverse proxy)
- **HTTPS Access**: https://n8n.hx.dev.local
- **Database**: PostgreSQL (n8n_poc3 database on hx-postgres-server)
- **Installation**: /opt/n8n/
- **Logs**: systemd journal (journalctl -u n8n)

**n8n Encryption Key**:
- **Key**: `N8N_ENCRYPTION_KEY` (64-character hex string, randomly generated)
- **Purpose**: Encrypts workflow credentials in PostgreSQL database
- **Generation**: `openssl rand -hex 32`
- **Storage**: `/opt/n8n/.env` (chmod 600, n8n user only)

**⚠️ CRITICAL - ENCRYPTION KEY BACKUP LOCATIONS**:

**Development Environment (POC3) - Minimum 2 Locations Required**:
- Location 1: Local access-restricted backup (required)
- Location 2: Password manager OR encrypted USB (required)
- Location 3: Offline/cloud storage (recommended for production)

**Production Environment - 3+ Locations Required**:
- All 3 locations mandatory for production deployments

---

**BACKUP LOCATION 1 - Local Access-Restricted Backup**:
- **Path**: `/opt/n8n/backups/.n8n-encryption-key.backup`
- **Permissions**: `chmod 600` (readable by n8n user only)
- **Owner**: `n8n:n8n`
- **Encryption**: **None** - relies on OS file permissions for protection
- **Purpose**: Local backup for disaster recovery within same server
- **Limitation**: ⚠️ If server is compromised, backup is accessible to attacker
- **Mitigation**: Use Location 2 (password manager) or Location 3 (offline/cloud) for secure backup
- **Verification**: Must be created before first n8n service startup

**BACKUP LOCATION 2 - Password Manager**:
- **Tool**: KeePassXC / 1Password / Bitwarden (organization standard)
- **Entry Title**: "n8n Encryption Key - hx-n8n-server.hx.dev.local"
- **Entry Type**: Secure Note
- **Metadata**: Include server IP (192.168.10.215), creation date, database name (n8n_poc3)
- **Purpose**: Secure off-server backup accessible to authorized personnel
- **Verification**: Entry must exist before first n8n service startup

**BACKUP LOCATION 3 - Offline/Cloud Secure Storage**:
- **Option A (Recommended)**: Encrypted USB drive in secure physical location
  - Encryption: AES-256 or equivalent
  - Storage location: Secure lockbox/safe
  - Label: "n8n Encryption Keys - DO NOT DESTROY"
- **Option B (Alternative)**: Organization's secure backup system
  - Cloud encrypted storage (e.g., AWS S3 with KMS, Azure Vault)
  - Encrypted with separate encryption key/passphrase
  - Access restricted to infrastructure administrators
- **Purpose**: Long-term archival and disaster recovery
- **Verification**: Backup must be confirmed accessible before first service startup

**🚨 DATA LOSS WARNING**:
- **WITHOUT ENCRYPTION KEY**: ALL encrypted credentials in n8n workflows become permanently unrecoverable
- **IMPACT**: Complete loss of all workflow authentication (API keys, passwords, tokens)
- **RECOVERY**: Impossible without the encryption key - workflows must be manually reconfigured
- **MANDATE**: All 3+ backup locations MUST be verified before first n8n startup

**Encryption Key Backup Procedure**:
```bash
# After generating encryption key in /opt/n8n/.env
# STEP 1: Create local encrypted backup
sudo cp /opt/n8n/.env /opt/n8n/backups/.n8n-encryption-key.backup
sudo chmod 600 /opt/n8n/backups/.n8n-encryption-key.backup
sudo chown n8n:n8n /opt/n8n/backups/.n8n-encryption-key.backup

# STEP 2: Extract encryption key value
sudo grep N8N_ENCRYPTION_KEY /opt/n8n/.env
# Copy the 64-character hex value

# STEP 3: Store in password manager
# (Manual step - add secure note with key value)

# STEP 4: Create offline backup
# (Manual step - write to encrypted USB or upload to secure cloud storage)

# STEP 5: Verify all backups accessible
# Test each backup location can be retrieved
```

**Encryption Key Recovery Test**:
```bash
# Annually verify encryption key recovery from all backup locations
# LOCATION 1 TEST:
sudo cat /opt/n8n/backups/.n8n-encryption-key.backup | grep N8N_ENCRYPTION_KEY

# LOCATION 2 TEST:
# Retrieve from password manager and verify matches production key

# LOCATION 3 TEST (if configured):
# Retrieve from offline/cloud storage and verify matches production key
```

**Recovery Test Failure Procedure**:

If any backup location is inaccessible during recovery test:

**🚨 CRITICAL - DO NOT START N8N SERVICE** - encrypted credentials will be inaccessible

**Immediate Actions**:
1. **Stop**: Do not proceed with service start
2. **Assess**: Identify which backup location(s) are inaccessible
3. **Recreate**: Immediately recreate missing backup from accessible location
4. **Verify**: Confirm all required backup locations accessible before resuming
5. **Document**: Record incident, root cause, and corrective action
6. **Update**: Revise backup procedures to prevent recurrence

**Example Failure Scenarios**:

| Failure Scenario | Recovery Action |
|------------------|-----------------|
| **Password manager access revoked** | Recreate entry from Location 1 or 3 |
| **Offline USB destroyed/lost** | Regenerate backup from Location 1 or 2 |
| **Server fails before backup created** | Use Location 2 or 3 to recover key |
| **All backups lost** | ⚠️ **UNRECOVERABLE** - all encrypted credentials permanently lost |

**Prevention**:
- Test all backup locations **monthly** (not annually) for production systems
- Implement automated backup verification alerts
- Maintain backup location redundancy (3+ for production)

**Login Example**:
```bash
# SSH to n8n server
ssh n8n@hx-n8n-server.hx.dev.local
# Password: Major8859!

# Verify domain account:
id n8n@hx.dev.local
getent passwd n8n@hx.dev.local
```
...
http://192.168.10.215:5678
```

---

### 15. Future Service Accounts (All Samba LDAP/DC)
...
| hx-n8n-server | 192.168.10.215 | agent0 | Major8859! | Administrator | Major3059! | n8n Workflow (n8n service user, POC3) |
...
| 2025-11-07 | Added n8n service account (UID TBD) with encryption key backup locations | POC3 n8n Workflow Automation deployment | Claude Code |
...
- All configuration documents reference these credentials
...
**Related Documents**:
- `0.0.5.2.0-readme.md` - Credentials Directory Overview
...
**Recent Changes:** Added n8n service account (POC3) with mandatory 3+ encryption key backup locations (local encrypted, password manager, offline/cloud storage) - Critical for data loss prevention
