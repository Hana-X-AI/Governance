...
**Supersedes**: Version 1.0 (2025-11-05)
**Changes**: POC-001 lessons learned integrated (knowledge source first, agent validation, mandatory reflection)
...
7. **Knowledge-Grounded**: Consult knowledge sources before drafting (NEW v1.1)
8. **Incremental Refinement**: Improve methodology after each POC/project (NEW v1.1)
9. **Aim Small, Miss Small**: Focus, precision, simplicity (NEW v1.1)

---

## Entry Point: Agent Zero as Universal Orchestrator

**All work begins with Agent Zero.**

Agent Zero is the Universal PM Orchestrator for the Hana-X ecosystem - the **single entry point** for ALL user requests (simple, medium, complex). Agent Zero executes this methodology, identifies specialist agents, coordinates planning, and validates outcomes.

**Workflow**: `User Request → Agent Zero → Work Methodology (6 phases) → Specialist Agents → Validated Outcome`

**Agent Zero's Role**:
- Analyze user request (what, why, expected outcome)
- Size task (simple/medium/complex)
- Execute appropriate methodology rigor
- Identify specialist agents via Agent Catalog
- Coordinate multi-agent planning (Phase 2)
- Validate outcomes (Phase 5)

**Why Agent Zero**: The 30 specialist agents are domain experts (Ollama, PostgreSQL, N8N, etc.). Agent Zero is the **planning and coordination expert** who orchestrates work across the ecosystem.

**See Agent Zero Profile**: `/srv/cc/Governance/0.1-agents/agent-zero.md`
...
```
Phase 0: Discovery       → Who? What? Which systems? (Agent Zero analyzes)
Phase 1: Specification   → What does success look like? (Agent Zero defines)
Phase 2: Planning        → Task breakdown, dependencies, coordination (Agent Zero facilitates)
Phase 3: Alignment       → Everyone ready? Rollback plan? (Agent Zero validates)
Phase 4: Execution       → Do the work, validate each step (Agent Zero coordinates)
Phase 5: Validation      → Tests pass? Docs updated? (Agent Zero verifies)
```
...
**Orchestration**: Agent Zero (orchestrating agent) scales rigor based on task complexity.
...
- **Agent Selection Guide**: `/srv/cc/Governance/0.0-governance/agent-selection-guide.md` - Decision flowchart (NEW v1.1)
- **Platform Nodes**: `/srv/cc/Governance/0.0-governance/0.2-hana_x_platform_nodes_final.md` - Server inventory

**NEW (v1.1): Step 0a - Knowledge Source Review**

**For work involving knowledge-expert agents** (agents with `/srv/knowledge/vault/` sources):

1. **Identify knowledge-expert agents** (William/nginx, Patricia/Ollama, Marcus/LightRAG, etc.)
2. **Review their knowledge sources BEFORE drafting specifications**:
   - Example: Nginx work → Review `/srv/knowledge/vault/nginx/`
   - Example: Ollama work → Review `/srv/knowledge/vault/ollama/`
3. **Ground specifications in expert knowledge** (not assumptions)

**Why This Matters** (POC-001 Lesson):
- Prevents gaps in specifications
- Reduces agent corrections during Phase 3
- Agents assist in planning up front (not just execution)
- Specifications reflect actual best practices from authoritative sources

**Example**:
```
Task: "Reload nginx configuration on hx-ssl-server"

Agent Zero (before drafting specification):
1. Identifies @agent-william (nginx operations)
2. Reviews /srv/knowledge/vault/nginx/ to understand:
   - nginx -t (config testing)
   - systemctl reload (SIGHUP vs SIGTERM)
   - journalctl validation (reload timestamp)
3. Drafts specification grounded in nginx best practices
4. William reviews and enhances (not corrects)
```
...
- **Knowledge sources reviewed** (if applicable) (NEW v1.1)
...
- Knowledge sources: /srv/knowledge/vault/ollama/ (model management)
...
**Agent Zero (orchestrating agent)** invites all identified agents:
...
Agent Zero (orchestrating agent) creates unified task list:
...
**NEW (v1.1): Agent Assignment Validation**

**Critical Safety Check**: Agents must validate they are the **CORRECT agent** for the work

**Each reviewing agent should confirm**:
1. **Domain Match**: "This work is within my domain expertise"
2. **Server Ownership**: "I own/manage the affected servers" (if applicable)
3. **Knowledge Source**: "I have the knowledge sources needed"

**If agent identifies misassignment**:
- Flag immediately: "This is not my domain - recommend @agent-X instead"
- Agent Zero reassigns and restarts Phase 3
- **This is a WIN, not a failure** - Phase 3 worked as designed

**POC-001 Success Example**:
```
@agent-frank reviewed nginx reload task and stated:
"This is not my domain (nginx operations).
This is hx-ssl-server (192.168.10.202) - not my assigned infrastructure.
Recommend @agent-william (Ubuntu Systems Administrator)"

Result: Agent Zero reassigned to William, execution succeeded.
Lesson: Phase 3 Alignment caught the error before execution.
```

**Why This Matters**:
- Agents know their boundaries (expertise and server ownership)
- Multi-agent review catches assignment errors
- Self-identification prevents wrong-agent execution
- Framework working as designed

**Output**: All agents confirm ✅ ready to proceed (and correct agent assigned)
...
□ This work IS within your domain (if not, flag immediately)
□ You have resources needed
□ You can commit to [timeline]
□ Rollback plan is clear

Reply with ✅ when ready (or flag if wrong agent assigned).
```
...
**NEW (v1.1): Mandatory Lessons Learned Session**

**REQUIRED for ALL POCs and medium/complex projects**:

After validation complete, conduct reflection session with CAIO:

**Lessons Learned Questions**:
1. **What went right?** (Process, decisions, coordination)
2. **What can improve?** (Gaps, inefficiencies, errors)
3. **What did we learn?** (New patterns, insights, validations)
4. **What should change?** (Update methodology, templates, guides)

**Document in completion summary** (section: "Lessons Learned")

**Update governance incrementally**:
...
- No premature optimization (don't add complexity until proven need)
- Incremental refinement over time

**POC-001 Example**:
- Lesson: Knowledge source review before drafting → Added to Phase 0
- Lesson: Agent assignment validation → Added to Phase 3
- Lesson: Mandatory reflection → Added to Phase 5

**Output**:
- All tests passed ✅
- Documentation updated ✅
- **Lessons learned session conducted** ✅ (NEW v1.1)
- **Methodology updated incrementally** ✅ (NEW v1.1)
...
- □ **Lessons learned captured** (NEW v1.1)
- □ **Knowledge sources reviewed** (if applicable) (NEW v1.1)
...
**Escalation Paths**:
- Infrastructure issues (LDAP/DNS/SSL) → @agent-frank → Agent Zero (if unresolved)
- OS/system issues → @agent-william → Agent Zero (if unresolved)
- Multi-agent coordination issues → @agent-george (FastMCP) → Agent Zero (if unresolved)
- Domain expertise → Consult Agent Catalog for specialist → Agent Zero (if unresolved)
- Governance/process unclear → This methodology + Constitution → Agent Zero (final authority)
- Unknown/complex → Agent Zero (direct escalation, final authority)

**Agent Zero is the terminal escalation point** - there is no further escalation beyond Agent Zero. Agent Zero makes final decisions on all governance, coordination, and complex issues.
...
| 1.1 | 2025-11-05 | Added Agent Zero as explicit entry point and universal orchestrator, updated escalation paths to show Agent Zero as terminal authority, clarified Agent Zero's role in all 6 phases | Agent Zero |
| 1.2 | 2025-11-06 | POC-001 lessons learned integrated: Knowledge source review (Phase 0), Agent assignment validation (Phase 3), Mandatory lessons learned (Phase 5), Added core principles #7-9 (Knowledge-Grounded, Incremental Refinement, Aim Small Miss Small) | Agent Zero |
...
- [Deployment Methodology](./0.0.1.2-deployment-methodology.md)  
- [Agent Selection Guide](./0.0.1.7-agent-selection-guide.md)  
**Classification**: Internal - Governance  
**Status**: ACTIVE - MANDATORY for all work in Hana-X ecosystem  
...
**Entry Point**: Agent Zero (all work begins here)
