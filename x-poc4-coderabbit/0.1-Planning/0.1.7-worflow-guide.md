...
**What You See**:
```
✅ Code fixed all critical issues recommended by CodeRabbit
✅ Ready to deploy
```
...
Claude: "I'll run CodeRabbit in the background and fix any issues found..."
        ...
        🐰 Roger: Starting CodeRabbit review...
        ...
        ⚠️  Found 5 issues:
            🔴 P0: Hardcoded API key in withings_client.py:23
            🔴 P0: Missing error handling in sync_weight_data()
            🟡 P1: Type hints missing in 3 functions
            ⚫ P2: Function complexity high in process_measurements()
            ⚫ P3: Missing docstring in WeightSync class
        ...
        ⚡ Improved: Refactored process_measurements()
        📝 Added: Complete docstrings
        
        🐰 Re-running CodeRabbit to verify...
        
        ✅ All critical issues resolved
        ✅ Code meets Hana-X standards
        ✅ Ready to deploy
...
### 3. Auto-Fix Loop
```
Review → Find Issues → Fix → Re-Review → Still Issues? → Fix Again
                                              ↓ No Issues
                                          ✅ Done
```
...
CI/CD Integration:
6. Run CodeRabbit on PR creation (not every commit)
7. Allow developers to run CodeRabbit locally before pushing
8. Gate CodeRabbit to periodic checks or merge-time validation
9. Use non-blocking mode for initial adoption (warn vs. fail)
10. Cache CodeRabbit results to avoid redundant scans
```
...
CI/CD Anti-Patterns:
5. Don't run CodeRabbit on every commit in CI (performance/cost impact)
6. Don't block all CI pipelines on CodeRabbit by default (slows feedback loop)
7. Don't run CodeRabbit multiple times for same code (cache results)
8. Don't fail builds for P2 issues during initial rollout (too disruptive)
9. Don't skip local validation before pushing (catches issues earlier)
```
...
Fix issues              → 30 min
Second review           → 30 min
Total: 4 hours
```

### With Roger + Claude Code
```
Write code              → 2 hours (Claude does it)
"Run CodeRabbit"        → 2 min (automated)
"Fix issues"            → 5 min (Claude does it)
Verify                  → 2 min
...
✅ Code reviews complete in < 5 minutes  
✅ P0/P1 issues caught before push  
✅ Team code quality improves over time  
✅ Less time in manual code review  
