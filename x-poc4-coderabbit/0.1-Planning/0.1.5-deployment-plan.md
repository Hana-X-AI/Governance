...
### Benefits
✅ Install Roger, quality gates, etc. **once**  
✅ New projects start in **minutes, not days**  
✅ Consistent quality standards across **all projects**  
✅ Updates to Roger benefit **all projects immediately**  
✅ Centralized defect tracking **across projects**  
...
class RogerGlobal:
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        print(f"🐰 Roger initialized for project: {self.project_name}")
    ...
    def review(self, auto_fix: bool = False, background: bool = False):
        ...
        print(f"\n🐰 Starting CodeRabbit review for {self.project_name}...")
        ...
        if auto_fix:
            print("🐰 Creating defects from findings...")
    ...
    def status(self):
        ...
        print(f"\n🐰 Roger Status for {self.project_name}")
...
echo "🚀 Bootstrapping new Hana-X project: $PROJECT_NAME"
echo ""
...
echo "📝 Generating project files from templates..."
...
- ✅ **Easier debugging**: Edit template files directly without escaping heredoc syntax
- ✅ **Better version control**: Template changes are clearly visible in git diffs
- ✅ **Simpler testing**: Test templates independently with sample variables
- ✅ **Extensibility**: Add new template files without modifying bootstrap script
- ✅ **IDE support**: Syntax highlighting works correctly in separate `.yaml`/`.md` files
- ✅ **Reusability**: Templates can be used by other tools (e.g., CI/CD pipelines)
...
# Should output:
# 🐰 Roger initialized for project: x-poc3-n8n-deployment
...
- ✅ Roger installed globally
- ✅ Quality gates shared across projects
- ✅ CodeRabbit CLI available everywhere
- ✅ Project bootstrap template ready

### Per-Project Setup (Phase 1 - 5 minutes)
- ✅ Link to shared infrastructure
- ✅ Project-specific CodeRabbit config
- ✅ Project defect log
- ✅ Done!
...
**Note on Document Structure**: This deployment plan is a **reference guide** containing executable bash commands. Users should extract and run commands manually or use the provided extraction instructions.

### How to Use This Deployment Plan

**Option 1: Manual Execution** (Recommended for first-time deployment)
```bash
# Copy and paste commands from each phase into your terminal
# Verify each step completes successfully before proceeding
# Example:
sudo mkdir -p /srv/cc/hana-x-infrastructure/.claude/agents/roger
cd /srv/cc/hana-x-infrastructure
# ... continue with next commands
```

**Option 2: Extract and Execute** (For experienced users)
```bash
# Extract bash commands from markdown code blocks
grep -A 100 '^```bash' /srv/cc/Governance/x-poc4-coderabbit/0.1-Planning/0.1.5-deployment-plan.md \
  | sed '/^```bash/d; /^```$/d' > deploy.sh

# Review the extracted script
less deploy.sh

# Execute with caution (review first!)
bash deploy.sh
```

**Option 3: Phase-by-Phase Extraction** (Most controlled approach)
```bash
# Extract only Phase 0 commands
sed -n '/## Phase 0/,/## Phase 1/p' /srv/cc/Governance/x-poc4-coderabbit/0.1-Planning/0.1.5-deployment-plan.md \
  | grep -A 100 '^```bash' | sed '/^```bash/d; /^```$/d' > phase0.sh
...
1. **Not Self-Executing**: This `.md` file cannot be executed directly with `bash filename.md`
2. **Manual Review Required**: Always review commands before execution
3. **Environment-Specific**: Paths and IPs may need adjustment for your environment
4. **Incremental Deployment**: Execute phases sequentially, validate each before proceeding

### Discoverability

This document is:
- ✅ Listed in project catalog: `/srv/cc/Governance/x-poc4-coderabbit/README.md`
- ✅ Referenced in Roger infrastructure docs
- ✅ Tagged with metadata for searchability (see below)

### Document Metadata

```yaml
type: deployment_plan
format: markdown_reference_guide
purpose: infrastructure_deployment
environment: multi_environment_capable
execution: manual_extraction_required
version: "2.0"
status: ready_for_implementation
maintainer: agent_zero
last_updated: "2025-11-10"
source_control: /srv/cc/Governance/x-poc4-coderabbit/0.1-Planning/0.1.5-deployment-plan.md
usage: extract_and_execute_bash_commands
file_type: .md (not directly executable)
```

**Conclusion**: This deployment plan is a reference guide containing executable bash commands. Users must extract and run commands manually or using the provided extraction instructions. The document is NOT self-executing and cannot be run directly as `bash filename.md`.
